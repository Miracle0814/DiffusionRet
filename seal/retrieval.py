...
class SEALSearcher:
    ...
    def batch_search(self, queries, k: int = 10, added_documents=None, detokenize=None) -> List[List[SEALDocument]]:
        ...
        gen_time = gen_end_time - start_time
        ...
        retrive_end_time = time.time()

        retrieve_time = retrive_end_time - gen_end_time
        ...
        total_time = total_end_time - retrive_end_time

        with open('/home2/qiaosb/SEAL/time_log_short_10.txt', mode='a') as sample:
            sample.write(str(gen_time) + '\t' + str(retrieve_time) + '\t' + str(total_time) + '\n')
